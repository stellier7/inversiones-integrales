const CART_STORAGE_KEY = 'buffalo-cart';
...
function saveCart() {
  ...
  document.dispatchEvent(new CustomEvent('buffalo:cart'));
}
...
function cartQuoteGreeting() {
  ...
  if (seller) {
    return `Hola ${seller.firstName}, quiero cotizar estos productos Buffalo y confirmar disponibilidad:`;
  }
  return 'Hola, quiero cotizar estos productos Buffalo y confirmar disponibilidad:';
}
...
function updateCartDestinationCopy() {
  ...
  if (sub) {
    sub.textContent = seller
      ...
      : 'Arma la lista y envíala por WhatsApp a Buffalo.';
  }
  if (quoteBtn) {
...
function initCart() {
  ...
  document.addEventListener('buffalo:cart', onCartChanged);
}
