const CART_STORAGE_KEY = 'lumiart-cart';
...
function saveCart() {
  ...
  document.dispatchEvent(new CustomEvent('lumiart:cart'));
}
...
function cartQuoteGreeting() {
  ...
  if (seller) {
    return `Hola ${seller.firstName}, quiero cotizar estos productos Lumiart y confirmar disponibilidad:`;
  }
  return 'Hola, quiero cotizar estos productos Lumiart y confirmar disponibilidad:';
}
...
function updateCartDestinationCopy() {
  ...
  if (sub) {
    sub.textContent = seller
      ...
      : 'Arma la lista y envíala por WhatsApp a Lumiart.';
  }
  if (quoteBtn) {
...
function initCart() {
  ...
  document.addEventListener('lumiart:cart', onCartChanged);
}
