const CART_STORAGE_KEY = 'trebol-cart';
...
function saveCart() {
  ...
  document.dispatchEvent(new CustomEvent('trebol:cart'));
}
...
function cartQuoteGreeting() {
  ...
  if (seller) {
    return `Hola ${seller.firstName}, quiero cotizar estos productos Trébol y confirmar disponibilidad:`;
  }
  return 'Hola, quiero cotizar estos productos Trébol y confirmar disponibilidad:';
}
...
function updateCartDestinationCopy() {
  ...
  if (sub) {
    sub.textContent = seller
      ...
      : 'Arma la lista y envíala por WhatsApp a Trébol.';
  }
  if (quoteBtn) {
...
function initCart() {
  ...
  document.addEventListener('trebol:cart', onCartChanged);
}
