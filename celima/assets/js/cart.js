const CART_STORAGE_KEY = 'celima-cart';
...
function saveCart() {
  ...
  document.dispatchEvent(new CustomEvent('celima:cart'));
}
...
function cartQuoteGreeting() {
  ...
  if (seller) {
    return `Hola ${seller.firstName}, quiero cotizar estos productos Celima y confirmar disponibilidad:`;
  }
  return 'Hola, quiero cotizar estos productos Celima y confirmar disponibilidad:';
}
...
function updateCartDestinationCopy() {
  ...
  if (sub) {
    sub.textContent = seller
      ...
      : 'Arma la lista y envíala por WhatsApp a Celima.';
  }
  if (quoteBtn) {
...
function initCart() {
  ...
  document.addEventListener('celima:cart', onCartChanged);
}
